...
if __name__ == "__main__":
    ...
    PATH_TO_CURRENT_OUT_FOLDER = os.path.join(PATH_TO_OUTPUT_FOLDER, 'ENG_AA',
                                              'TRAIN_6000_BOOTSTRAP_{}_SENTENCES'.format(COUNT_OF_SENTENCES))
                                              #'TRAIN_FIXED_SEPARATION_{}_SENTENCES'.format(COUNT_OF_SENTENCES))
    ...
    if TEST_AVALABLE:
        PATH_TO_CURRENT_OUT_TEST = os.path.join(PATH_TO_OUTPUT_FOLDER, 'ENG_AA',
                                                'TEST_FIXED_SEPARATION_{}_SENTENCES'.format(COUNT_OF_SENTENCES))
    ...
    if FAIR_TEST:
        PATH_TO_CURRENT_OUT_FAIR_TEST = os.path.join(PATH_TO_OUTPUT_FOLDER, 'ENG_AA',
                                                'TEST_SAMPLE_0.1_PERCENT_{}_SENTENCES'.format(COUNT_OF_SENTENCES))
        fullFairTestDatasetDocs_1 = pickle.load(open(os.path.join(PATH_TO_CURRENT_OUT_FAIR_TEST, 'tagged_documents_dump_1.pkl'),
                                               'rb'))
        print('Loading test processed documents... DONE ({} documents)'.format(len(fullFairTestDatasetDocs_1)))

        PATH_TO_CURRENT_OUT_FAIR_TEST = os.path.join(PATH_TO_OUTPUT_FOLDER, 'ENG_AA',
                                                'TEST_SAMPLE_0.1_PERCENT_{}_SENTENCES'.format(COUNT_OF_SENTENCES))
        fullFairTestDatasetDocs_2 = pickle.load(open(os.path.join(PATH_TO_CURRENT_OUT_FAIR_TEST, 'tagged_documents_dump_2.pkl'),
                                               'rb'))
        print('Loading test processed documents... DONE ({} documents)'.format(len(fullFairTestDatasetDocs_2)))
        fullFairTestDatasetDocs = fullFairTestDatasetDocs_1 + fullFairTestDatasetDocs_2
        print('Concatenation... DONE ({} documents)'.format(len(fullFairTestDatasetDocs)))
